...
# Black line length (default 88, we use 100)
...
# Local modules third
from transcription.utils import load_model
...
def transcribe(file: str) -> str:
    result = process_file(file)  # Internal variables can wait
    return result

# Gradually add more detail
def transcribe(file: str) -> str:
    result: dict[str, Any] = process_file(file)
    return result["text"]
```
